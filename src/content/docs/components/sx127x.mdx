...
- **dio1_pin** (**Optional**, [Pin Schema](/guides/configuration-types#pin-schema)): Digital IO pin 1,
  mapped to the chip's FifoLevel signal. Required when FSK/OOK `payload_length` exceeds the 64-byte FIFO;
  the component uses this interrupt to stream large packets through the FIFO. When wired, variable
  length FSK/OOK packets can also receive up to 255 bytes.
...
- **payload_length** (**Optional**, int): Length of the payload in packet mode. If not configured then
  the variable length packet format is used (maximum 63 bytes, or 255 when `dio1_pin` is set). In
  fixed length mode the maximum is 64 bytes, or 2047 when `dio1_pin` is set.
