...
- **dio1_pin** (**Optional**, [Pin Schema](/guides/configuration-types#pin-schema)): Digital IO pin 1,
  mapped to the chip's FifoLevel signal. Required when `payload_length` exceeds the 64-byte FIFO; the
  component uses this interrupt to stream large packets through the FIFO. When wired, the variable
  length format can also receive up to 255 bytes per packet.
